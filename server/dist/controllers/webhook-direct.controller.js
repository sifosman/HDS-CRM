        ...
        try {
            ...
            if (ocrText) {
                ...
                try {
                    ...
                    const extractionResult = (0, ocr_disabled_service_1.extractDimensionsFromText)(ocrText);
            ...
            console.log('📋 WEBHOOK: Cut pieces summary:', cutPieces.map((cp) => `${cp.length}x${cp.width}(${cp.quantity})`).join(', '));
            ...
            try {
                const saveResult = await supabase_service_1.default.saveCutlist(cutlistData);
            ...
            try {
                ...
                for (const code of productCodes) {
                    try {
                        const pricing = await supabase_service_1.default.getProductPricing(code);
                ...
                if (pricingData.length > 0) {
                    try {
                        ...
                        const quoteResult = await supabase_service_1.default.createQuote(quoteData);
