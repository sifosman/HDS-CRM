...
const isWhiteMelamine = (material) => {
...
const convertUnit = (value, fromUnit, toUnit) => {
    ...
    if (fromUnit === 0) { // from mm
        ...
        if (toUnit === 2)
            return value * mmToFoot;
    }
    else if (fromUnit === 1) { // from inch
        if (toUnit === 0)
            return value * inchToMm;
        if (toUnit === 2)
            return value * inchToFoot;
    }
    else if (fromUnit === 2) { // from foot
        if (toUnit === 0)
            return value * footToMm;
        if (toUnit === 1)
            return value * footToInch;
    }
    return value;
};
// Prepare data for optimization
const prepareOptimizationData = (pieces, unit) => {
    const stockPieces = [];
    const cutPieces = [];
    let seq = 0;
    pieces.forEach(piece => {
        ...
        if (piece.kind === 1) { // Stock piece
            ...
            for (let i = 0; i < piece.amount; i++) {
                ...
                const anyPiece = piece;
                const rawEdging = anyPiece === null || anyPiece === void 0 ? void 0 : anyPiece.edging;
                ...
                if (rawEdging !== undefined && rawEdging !== null) {
                    if (rawEdging === 1 || rawEdging === true || (typeof rawEdging === 'string' && rawEdging.trim() === '1')) {
                        ...
                        rawEdging.split(',').map((s) => s.trim()).filter((s) => s).forEach((s) => {
                            if (s === 'L1')
                                edgeL1 = true;
                            if (s === 'L2')
                                edgeL2 = true;
...
exports.prepareOptimizationData = prepareOptimizationData;
// Improved optimization function with better bin packing algorithm
const optimizeCuttingLayout = (stockPieces, cutPieces, cutWidth, layout) => {
    const solution = {
        stockPieces: []
    };
                ...
                for (let j = 0; j < freeRects.length; j++) {
                    ...
                    if (canPlaceNormal || canPlaceRotated) {
                        ...
                        const placedPiece = {
                            ...
                            edging: cutPiece.edging,
                            edgeL1: cutPiece.edgeL1,
                            edgeL2: cutPiece.edgeL2,
...
exports.optimizeCuttingLayout = optimizeCuttingLayout;
// Generate PDF with the solution
const generatePdf = (solution, unit, cutWidth = 3, layout = 0) => {
    const pdfId = (0, uuid_1.v4)();
    const pdfPath = path_1.default.join(__dirname, '../../pdfs', `solution_${pdfId}.pdf`);
    // Ensure directory exists
    const dir = path_1.default.dirname(pdfPath);
    ...
    solution.stockPieces.forEach((stockPiece, index) => {
        ...
        doc.fontSize(14)
            .fillColor('#FFFFFF')
            .text(`CASE ${index + 1}`, stockTitleX, stockTitleY + 8, { align: 'center', width: stockTitleWidth });
        ...
        // Draw header
        doc.rect(stockDetailsStartX, stockDetailsStartY, stockDetailsColWidths.reduce((a, b) => a + b, 0), stockDetailsRowHeight)
            .fillAndStroke('#e0e0e0', '#000000');
        doc.fontSize(10).fillColor('#000000');
        doc.text('Resource', stockDetailsStartX + 5, stockDetailsStartY + 8, { width: stockDetailsColWidths[0] });
        doc.text('Width', stockDetailsStartX + stockDetailsColWidths[0] + 5, stockDetailsStartY + 8, { width: stockDetailsColWidths[1] });
        doc.text('Length', stockDetailsStartX + stockDetailsColWidths[0] +
            stockDetailsColWidths[1] + 5, stockDetailsStartY + 8, { width: stockDetailsColWidths[2] });
        doc.text('Area', stockDetailsStartX + stockDetailsColWidths[0] +
            stockDetailsColWidths[1] + stockDetailsColWidths[2] + 5, stockDetailsStartY + 8, { width: stockDetailsColWidths[3] });
        // Draw data row
        const stockDetailsDataY = stockDetailsStartY + stockDetailsRowHeight;
        doc.rect(stockDetailsStartX, stockDetailsDataY, stockDetailsColWidths.reduce((a, b) => a + b, 0), stockDetailsRowHeight)
            .stroke();
        ...
        doc.text(`Case ${index + 1}`, stockDetailsStartX + 5, stockDetailsDataY + 8, { width: stockDetailsColWidths[0] });
        ...
        const scale = Math.min(pageWidth / stockPiece.width, pageHeight / stockPiece.length) * 0.8;
        // Draw stock piece
        const startX = 50;
        const startY = 120;
        // Draw stock piece outline
        doc.rect(startX, startY, stockPiece.width * scale, stockPiece.length * scale).stroke('#000000');
        // Draw cut pieces
        stockPiece.cutPieces.forEach((cutPiece, pieceIndex) => {
            ...
            // Draw cut piece
            doc.rect(startX + cutPiece.x * scale, startY + cutPiece.y * scale, cutPiece.width * scale, cutPiece.length * scale).fillAndStroke(fillColor, '#000000');
            ...
            doc.fontSize(14)
                .fillColor('#000000')
                .text(`${cutPiece.externalId}`, labelX - 10, labelY - 10, { width: 20, align: 'center' });
            // Draw width dimension on top
            const widthLabel = convertUnit(cutPiece.width, 0, unit).toFixed(0);
            doc.fontSize(8)
                .fillColor('#000000')
                .text(widthLabel, startX + cutPiece.x * scale + (cutPiece.width * scale / 2) - 10, startY + cutPiece.y * scale - 12, { width: 20, align: 'center' });
            // Draw length dimension on the left side
            const lengthLabel = convertUnit(cutPiece.length, 0, unit).toFixed(0);
            doc.fontSize(8)
                .fillColor('#000000')
                .text(lengthLabel, startX + cutPiece.x * scale - 20, startY + cutPiece.y * scale + (cutPiece.length * scale / 2) - 5, { width: 20, align: 'center' });
        ...
        doc.fontSize(14)
            .fillColor('#FFFFFF')
            .text('CUTTING PARAMETERS', infoTitleX, infoTitleY + 8, { align: 'center', width: infoTitleWidth });
        doc.moveDown(2);
    ...
    doc.text(`HDS Group Cutlist - Generated on ${dateString}`, 50, doc.page.height - 50, { align: 'center', width: doc.page.width - 100 });
    // Finalize PDF
    doc.end();
    return pdfId;
};
...
const generateInvoicePdf = (quoteData, branchData) => {
        ...
        try {
            ...
            try {
                ...
                if (sections && sections.length > 0) {
                    ...
                    const chargeableBoardsUsed = sections.reduce((sum, section) => sum + (isWhiteMelamine(section.material) ? 0 : (section.boardsNeeded || 0)), 0);
            ...
            const totalBoardsUsed = sections.length > 0
                ? sections.reduce((sum, section) => sum + (section.boardsNeeded || 0), 0)
            ...
            const chargeableBoardsUsed = sections.length > 0
                ? sections.reduce((sum, section) => sum + (isWhiteMelamine(section.material) ? 0 : (section.boardsNeeded || 0)), 0)
                : totalBoardsUsed;
...
const importFromIQ = (iqData) => {
    ...
    if (Array.isArray(iqData.stockPieces)) {
        iqData.stockPieces.forEach((stockPiece) => {
...
const generateQuotePdf = (quoteData, isPaid = false) => {
    const { quoteId, customerName, projectName, date, sections, grandTotal, branchData, bankingDetails, phoneNumber, hardwareItems, hardwareTotal } = quoteData;
    ...
    if (hardwareItems && Array.isArray(hardwareItems) && hardwareItems.length > 0) {
        ...
        hwy += 22;
        // Data rows
        hardwareItems.forEach((hwItem, idx) => {
    ...
    if (isPaid) {
        ...
        doc.fontSize(8).fillColor(COLOR_TEXT_GRAY).font('Helvetica-Bold');
        doc.text('CLICK THE BUTTON BELOW TO PAY ONLINE NOW', MARGIN, y, { width: CONTENT_W, align: 'center' });
        y += 14;
        ...
        doc.text('Secure online payment via PayFast  \u2022  Card, EFT & Instant EFT accepted  \u2022  Click to pay immediately', MARGIN, y, { width: CONTENT_W, align: 'center' });
        y += 16;
    }
    // ====== 7. DISCLAIMER ======
    y = ensureSpace(30, y);
...
const generatePdfWithBuffer = async (solution, unit, cutWidth = 3, layout = 0) => {
        ...
        stockPiece.cutPieces.forEach((cutPiece, cutIndex) => {
            ...
            if (cpAny && (cpAny.edgeL1 || cpAny.edgeL2 || cpAny.edgeW1 || cpAny.edgeW2)) {
                ...
                if (cpAny.edging === 1 || cpAny.edging === true) {
                    ...
                    cpAny.edging.split(',').map((s) => s.trim()).filter((s) => s).forEach((s) => edgingSides.push(s));
                }
            }
    ...
    try {
        ...
        doc.fontSize(8).fillColor('#666666');
        doc.text(`HDS Group Cutlist - Generated on ${new Date().toLocaleDateString()}`, 50, doc.page.height - 50, { align: 'center', width: doc.page.width - 100 });
