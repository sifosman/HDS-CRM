...
const SupabaseService = {
     ...
     */
    async getHardwarePricing(productName) {
        try {
            ...
            if ((!data || data.length === 0) && !error) {
                ...
                if (tokens.length > 0) {
                    ...
                    if (data && data.length > 0) {
                        ...
                        const searchTokensLower = tokens.map((t) => t.toLowerCase());
                        ...
                        const goodMatches = scored.filter((s) => s.score >= minScore);
                        if (goodMatches.length > 0) {
                            goodMatches.sort((a, b) => b.score - a.score);
                            data = goodMatches.map((s) => s.product);
            ...
            const isHardwareProduct = (product) => {
                if (!product || !Array.isArray(product.categories)) {
                    ...
                    if (product.is_manufactured === true)
                        return false;
                    return true;
                }
                const categorySlugs = product.categories.map((c) => c === null || c === void 0 ? void 0 : c.slug).filter(Boolean);
                ...
                if (categorySlugs.some((s) => boardSlugs.includes(s))) {
                    return false;
                }
                // Include if it has a hardware category slug
                if (categorySlugs.some((s) => hardwareCategorySlugs.includes(s))) {
                    return true;
                }
     ...
     */
    async getRecentChatbotQuote(phoneNumber, withinMinutes = 30) {
        try {
            if (!phoneNumber)
     ...
     */
    async countRecentChatbotQuotes(phoneNumber, withinMinutes = 30) {
        try {
            if (!phoneNumber)
                return 0;
            let normalized = phoneNumber.replace(/^\+/, '');
     ...
     */
    async updateExistingQuote(quoteNumber, updateData) {
        try {
            const updatePayload = {
                updated_at: new Date().toISOString(),
            };
            if (updateData.quoteData !== undefined)
                updatePayload.quote_data = updateData.quoteData;
            if (updateData.subtotal !== undefined)
                updatePayload.subtotal = updateData.subtotal;
            if (updateData.tax !== undefined)
                updatePayload.tax = updateData.tax;
            if (updateData.total !== undefined)
                updatePayload.total = updateData.total;
            if (updateData.cutlistUrl !== undefined)
                updatePayload.cutlist_url = updateData.cutlistUrl;
            if (updateData.cutlistPdfUrl !== undefined) {
                // Only update cutlist_url if cutlistUrl wasn't explicitly set
                if (updateData.cutlistUrl === undefined)
                    updatePayload.cutlist_url = updateData.cutlistPdfUrl;
            }
            if (updateData.filename !== undefined)
                updatePayload.filename = updateData.filename;
            if (updateData.cutlistId !== undefined)
                updatePayload.cutlist_id = updateData.cutlistId;
            const { data, error } = await supabase
                .from('quotes')
     ...
     */
    async assignBranchToQuote(quoteNumber, branchData) {
        try {
            if (!quoteNumber) {
