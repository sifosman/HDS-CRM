...
class EmailService {
    ...
    generatePlainTextPaymentConfirmation(data) {
        const optimization = data.optimizationDetails || {};
        const lines = [];
        ...
        if (data.customerPhone)
            lines.push(`Contact Number: ${data.customerPhone}`);
        ...
        if (optimization.totalBoards !== undefined)
            lines.push(`Total Boards: ${optimization.totalBoards}`);
        if (optimization.totalLength !== undefined)
            lines.push(`Total Length: ${optimization.totalLength}mm`);
        if (optimization.wastage !== undefined)
            lines.push(`Wastage: ${optimization.wastage}%`);
        if (optimization.cutlistUrl)
            lines.push(`Quote Link: ${optimization.cutlistUrl}`);
        lines.push('');
        lines.push('Download Documents');
        if (data.quotePdfUrl)
            lines.push(`Quote PDF: ${data.quotePdfUrl}`);
        if (data.invoicePdfUrl)
            lines.push(`Invoice PDF: ${data.invoicePdfUrl}`);
        if (data.cutlistPdfUrl)
            lines.push(`Cutlist PDF: ${data.cutlistPdfUrl}`);
        if (!data.quotePdfUrl && !data.invoicePdfUrl && !data.cutlistPdfUrl)
            lines.push('Documents will be available shortly.');
        lines.push('');
    ...
    generatePlainTextQuoteCreated(data) {
        const lines = [];
        ...
        if (data.customerPhone)
            lines.push(`Customer Phone: ${data.customerPhone}`);
        if (data.projectName)
