...
const SupabaseService = {
   ...
   */
  async getProductPricingByDescription(description: string, includeSizes: boolean = false, priceList: string = 'default'): Promise<{
    success: boolean;
    error?: string;
    data?: {
      description: string;
   ...
   * @param priceList Optional pricing list selector: 'william' uses hds_prices_william, default uses hds_prices
   */
  async getMaterialOptions(priceList: string = 'default'): Promise<any> {
    try {
      const tableName = priceList === 'william' ? 'hds_prices_william' : 'hds_prices';
      const { data, error } = await supabase
        .from(tableName)
   ...
   * @param priceList Optional pricing list selector: 'william' uses hds_prices_william, default uses hds_prices
   */
  async getProductDescriptions(priceList: string = 'default'): Promise<any> {
    try {
      const tableName = priceList === 'william' ? 'hds_prices_william' : 'hds_prices';
   ...
   */
  async setPaymentMethodOnQuote(quoteNumber: string, paymentMethod: string): Promise<any> {
    try {
      if (!quoteNumber) {
        return { success: false, error: 'Missing quoteNumber' };
      }
    ...
    try {
      ...
      if (parts.length >= 4) {
        ...
        if (!listErr && allBranches && allBranches.length > 0) {
          ...
          const deriveVercelAbbr = (name: string) => {
            const words = name.split(/\s+/).filter(w => w.length > 0);
            if (words.length === 1) return words[0].substring(0, 8).toUpperCase();
            if (words.length === 2) return (words[0].substring(0, 4) + words[1].substring(0, 4)).toUpperCase();
   ...
   */
  async fetchInvoiceByQuoteId(quoteIdentifier: string): Promise<any> {
    try {
