...
export const debugPayFastSignature = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const signature = crypto.createHash('md5').update(stringWithPassphrase).digest('hex');
    ...
    const completeParamString = Object.keys(completeData)
      .map(key => `${key}=${completeData[key as keyof typeof completeData]}`)
      .join('&');
    ...
    const completeSignature = crypto.createHash('md5').update(completeStringWithPassphrase).digest('hex');
